...
use App\Models\HupkastPlatforme;
...
class HuPkastController extends Controller
{
    ...
    public function getLinks($platform)
    {
        $link = null;
        switch ($platform) {
            case 'youtube':
                $feed = FeedsFacade::make('https://fetchrss.com/feed/1vuDbF2wp3Sl1vuDby4MG4ry.rss');
                $items = $feed->get_items();
            ...
            case 'podcast.rs':
                $feed = FeedsFacade::make('https://fetchrss.com/feed/1vuDbF2wp3Sl1vuEHgCG14l1.rss');
                $items = $feed->get_items();
                break;
            default:
                # code...
        ...
        foreach ($arr as $item) {
            if (!$this->checkIfHuPkastExists($item)) {
                ...
                try {
                    ...
                    $tekst->published_at =  Carbon::createFromFormat('d F Y, h:i a', $item->published_at)
                        ->format('Y-m-d H:i:s');
                    ...
                    if ($tekst->save()) {
                        ...
                        if ($hupkast->save()) {
                            ...
                            // $platformaid = HupkastPlatforme::where('naziv_platforme', 'YouTube')->firstOrFail();
                            // $link = $this->getLinks('podcast.rs');
                            // $hupkast->linkovi()->attach($platformaid, [$link]);
                        }
                    }
                } catch (\Exception $e) {
